...
bool send_midi_report(usb_report_t report) {
	static uint8_t piano_channel = 0;

	midi_report_t *midi_report = (midi_report_t *)report.data;

	for (int i = 0; i < OCTAVE_COUNT; ++i) {
		for (int j = 0; j < 13; j++) {
			bool last_active = last_report.keys[i] & (1 << j);
			bool active = midi_report->keys[i] & (1 << j);
	
			if (active != last_active) {
				set_note(piano_channel, tu_min8((((MIDI_SHIFT) + i) * 12) + j, 127), active);
			}
		}
	}
